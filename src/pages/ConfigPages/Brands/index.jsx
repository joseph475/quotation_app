...
class Brands extends Component {
  ...
  handleYes = () => {
    deleteData(this.state.idForDeletion, tbl_brands)
    this.fetchData()
  };
  ...
  async fetchData() {
    const strgModels = fetchLocalStorage(tbl_brands);

    if (!strgModels) {
      await fetchDataFromAPI(tbl_brands).then(() => {
        this.setState({
          loading: false 
        });
      });
    ...
    this.setState({
      rawData: fetchLocalStorage(tbl_brands),
      filteredItems: fetchLocalStorage(tbl_brands),
      loading: false
    })
  }
      ...
      <div>
        ...
        {isAddEditSingleItemModalOpen && 
         <AddEditSingleItemModal
            ...
            modalTitle={itemForUpdate ? 'Update Brand' : 'Add New Brand'}
         />
        }
      </div>
    )
